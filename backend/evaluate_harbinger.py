...
Purpose
-------
Evaluates the operational effectiveness of Harbinger's adaptive checkpoint
optimization strategy using a synthetic cluster-scale workload simulation.
The framework compares adaptive checkpoint selection against multiple static
checkpoint policies and reports aggregate operational cost across simulated
AI training jobs.
...
- Cluster-scale operational savings

Functions
---------
calc_cost(job, interval)
    Computes the operational cost of a checkpoint policy by combining
    checkpoint overhead and expected recovery loss.

Processing Stages
-----------------
1. Generate synthetic workload.
2. Evaluate static checkpoint policies.
3. Evaluate adaptive Harbinger policy.
4. Aggregate cluster-scale operational cost.
